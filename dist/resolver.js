(function (factory) {
    ...
    class Resolver {
        ...
        register(url) {
            ...
            if (module) {
                return promise = new Promise(resolve => module.then((m) => resolve(m)));
            }
            else {
                if (ignores[url]) {
                    var callback = new Function(`return ${ignores[url]};`);
                    var v = {};
                    return module = modules[url] = new Promise(resolve => resolve({ module: callback }));
                }
                else {
        ...
        define(uris, callback) {
            ...
            new Promise((resolve, reject) => {
                ...
                Promise.all(mixin_1.map(uris, (uri) => {
                    ...
                    setModule && setModule({ module: callback });
                });
            });
        }
        resolve(uri) {
